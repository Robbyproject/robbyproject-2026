...
import CardSpotlight from "@/components/ui/CardSpotlight"; // Import Spotlight
...
export default function ChatBot({ isOpen, onClose }) {
  ...
  const handleSend = async () => {
    ...
    // Simulasi Balasan Bot (Bisa diganti API nanti)
    setTimeout(() => {
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="fixed bottom-24 right-6 lg:bottom-28 lg:right-10 w-[90vw] md:w-[400px] h-[500px] z-50 rounded-2xl overflow-hidden shadow-2xl"
        >
          {/* --- WRAPPER SPOTLIGHT --- */}
          <CardSpotlight 
            color="rgba(255, 255, 255, 0.1)" // Cahaya Putih Halus
                  ...
                  <div className={`flex items-end gap-2 max-w-[80%] ${msg.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
                    ...
                    <div 
                      className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${
                        msg.role === "user" 
                          ? "bg-zinc-200 text-black rounded-br-none font-medium" 
                          : "bg-zinc-800/80 text-zinc-300 border border-white/5 rounded-bl-none"
                      }`}
                    >
                      {msg.text}
                    </div>
              ...
              >
                <div className="relative flex-1">
                    <input 
                      ...
                      className="w-full bg-zinc-900/50 border border-zinc-700/50 text-white text-sm rounded-full py-2.5 pl-4 pr-10 focus:outline-none focus:border-zinc-500 transition-colors placeholder:text-zinc-600"
                    />
