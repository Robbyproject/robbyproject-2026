...
function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  // 2. AUTO SCROLL TO TOP saat ganti halaman
  useEffect(() => {
    ...
    <div className="flex w-full h-screen bg-zinc-50 dark:bg-[#0a0a0a] overflow-hidden">
      ...
      <div className="flex-1 relative flex flex-col h-full min-w-0 bg-zinc-50 dark:bg-[#0a0a0a] lg:ml-64 transition-all duration-300 ease-in-out">
        ...
        >
          {/* Progress Bar Sticky */}
          <motion.div
            className="sticky top-0 left-0 right-0 h-[3px] bg-emerald-500 z-[50] origin-left shadow-[0_0_10px_rgba(16,185,129,0.5)]"
            style={{ scaleX }}
          />
          ...
          <AnimatePresence>
            {showScrollBtn && (
              <motion.button
                ...
                onClick={scrollToTop}
                className="fixed bottom-8 right-8 z-[100] p-3 rounded-full bg-emerald-500 text-white shadow-lg shadow-emerald-500/30 hover:bg-emerald-600 transition-colors"
              >
                <ArrowUp size={20} />
              </motion.button>
            )}
