...
import { initialGallery, type GalleryItem } from "@/data";
...
import {
  SiHtml5, SiJavascript, SiPhp, SiMysql,
  SiReact, SiNextdotjs, SiTailwindcss
} from "react-icons/si";
import { FaPaintBrush, FaPen, FaCss3 } from "react-icons/fa";
...
const techIcons: Record<string, TechIconInfo> = {
  "Photoshop": { icon: FaPaintBrush, color: "#31A8FF" },
  "Illustrator": { icon: FaPen, color: "#FF9A00" },
            ...
            >
              ...
              >
                  ...
                  <div className="absolute inset-0 bg-black/40 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center">
                      <button className="flex items-center gap-2 bg-white text-black px-3 py-2 sm:px-5 sm:py-2.5 rounded-full font-bold text-xs transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300 shadow-xl hover:scale-105">
                              ...
                              if (TechInfo) {
                                  return (
                                      <div key={index} className="relative group/icon cursor-help">
                                          <TechInfo.icon size={16} className="text-zinc-400 dark:text-zinc-500 hover:text-[var(--icon-color)] transition-colors" style={{ "--icon-color": TechInfo.color } as React.CSSProperties} />
                                      </div>
                                  );
                              }
                              return null;
